...
4. **Corrections.** A correction is held to four tests of its own: wave B may retrace at most 105% of wave A, or 138.2% with expanded flats enabled; wave C must run 0.382-4.236 of wave A; a completed A-B-C **must finish on the correcting side of where it started**, and the whole correction **must hold the origin of the impulse it corrects**. Those last two are what stop a rally past the wave 5 top being dressed up as an A-B-C, and stop a decline that erases the entire impulse being called a correction of it. A move that retraces more than 100% is not correcting anything: it is a new impulse in the other direction, and it is counted as one, labelled 1-2-3-4-5 straight off the top with no letters at all. Contracting triangles are held to the same containment rule: every corner must stay on the correcting side of the start.
   ...
   **Telling a correction from a new impulse.** Down-up-down after a top reads either as A-B-C or as waves 1-2-3 of a new decline, and the two frequently score the same, so a tie-break decides. The tie-break is the oldest test in Elliott: subdivision. An impulsive leg breaks into five waves at the next degree down, a corrective one into three. The script measures the first leg after the impulse against the lower degree swing set; five sub-waves lean the count towards a new impulse, three keep it a correction, and anything unreadable falls back to preferring the correction.

   A second read on the same question is **velocity**. A correction is the market resting, so it should not cover ground faster than the impulse it is undoing. The first leg after the impulse is compared with the mean velocity of waves 1, 3 and 5: more than 1.2 times faster reads as impulsive, less than 0.8 times reads as corrective, and more than twice as fast forces the impulse reading wherever one is valid. Subdivision is the stronger evidence and is weighted double, so velocity mainly decides the cases where subdivision cannot tell - which on real charts is most of them. This runs off the lower degree whether or not you have it displayed, so **Lower degree size** affects your counts even with the labels hidden - make it too coarse and the subdivision test just returns "cannot tell".
