"""OHLC loading and validation helpers."""
...
def load_ohlc_csv(path: str, time_column: str | None = "time") -> pd.DataFrame:
    ...
    if time_column and time_column.lower() in df.columns:
        parsed = pd.to_datetime(df[time_column.lower()], unit="s", errors="coerce")
        if parsed.isna().all():
            parsed = pd.to_datetime(df[time_column.lower()], errors="coerce")
        df[time_column.lower()] = parsed
